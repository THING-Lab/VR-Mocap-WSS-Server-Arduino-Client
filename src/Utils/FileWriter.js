...
var writeToDisk = function(wemos_uuid_file, wemos_uuid, wemos_uuid_qtm_file, wemos_uuid_qtm){
    try {
        console.log('Writing to disk Path: ' + path);
        fs.writeFile(path + wemos_uuid_file, JSON.stringify(wemos_uuid) , 'utf-8', function(err, result) {
            if(err) console.log('error', err);
          });
        fs.writeFile(path + wemos_uuid_qtm_file, JSON.stringify(wemos_uuid_qtm) , 'utf-8',function(err, result) {
            if(err) console.log('error', err);
          });
    } catch (error) {
